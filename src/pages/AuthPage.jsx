...
export default function AuthPage() {
  ...
  const T = isDark ? {
    ...
    googleHover: 'rgba(255,255,255,0.08)',
    themeBg: 'rgba(255,255,255,0.04)',
    themeBorder: 'rgba(255,255,255,0.08)',
    themeActive: 'rgba(99,102,241,0.4)',
    ...
    googleBg: 'rgba(0,0,0,0.03)',
    googleBorder: 'rgba(0,0,0,0.1)',
    googleHover: 'rgba(0,0,0,0.06)',
    themeBg: 'rgba(0,0,0,0.04)',
    themeBorder: 'rgba(0,0,0,0.08)',
    themeActive: 'rgba(99,102,241,0.15)',
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Inter', 'DM Sans', sans-serif", padding: 20, position: 'relative', overflow: 'hidden', transition: 'background .3s' }}>
      ...
      {/* Background orbs */}
      <div style={{ position: 'absolute', top: '-10%', left: '-5%', width: 400, height: 400, borderRadius: '50%', background: `radial-gradient(circle, ${T.orb1} 0%, transparent 70%)`, pointerEvents: 'none', transition: 'all .3s' }} />
      <div style={{ position: 'absolute', bottom: '-10%', right: '-5%', width: 500, height: 500, borderRadius: '50%', background: `radial-gradient(circle, ${T.orb2} 0%, transparent 70%)`, pointerEvents: 'none', transition: 'all .3s' }} />
      <div style={{ position: 'absolute', top: '40%', left: '60%', width: 300, height: 300, borderRadius: '50%', background: `radial-gradient(circle, ${T.orb3} 0%, transparent 70%)`, pointerEvents: 'none', transition: 'all .3s' }} />

      {/* Theme toggle — canto superior direito */}
      <div style={{ position: 'fixed', top: 20, right: 20, display: 'flex', gap: 4, background: T.themeBg, border: `1px solid ${T.themeBorder}`, borderRadius: 12, padding: 4, backdropFilter: 'blur(10px)', zIndex: 10 }}>
        {THEMES.map(([k, ic, lb]) => (
          <button key={k} onClick={() => setTheme(k)} title={lb}
            style={{ background: theme === k ? T.themeActive : 'transparent', border: theme === k ? `1px solid rgba(99,102,241,0.4)` : '1px solid transparent', borderRadius: 8, padding: '5px 9px', cursor: 'pointer', fontSize: 13, transition: 'all .2s', lineHeight: 1 }}>
            {ic}
          </button>
        ))}
      </div>

      <div style={{ background: T.card, border: `1px solid ${T.cardBorder}`, borderRadius: 22, padding: '38px 38px 34px', width: '100%', maxWidth: 420, boxShadow: T.shadow + ', 0 0 0 1px rgba(255,255,255,0.04)', position: 'relative', backdropFilter: 'blur(20px)', transition: 'background .3s, border-color .3s' }}>
        {/* Top accent line */}
        ...
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.5px', color: T.text, marginBottom: 4, transition: 'color .3s' }}>FinanceControl</div>
          <div style={{ fontSize: 11, color: T.textMuted, fontFamily: 'monospace', transition: 'color .3s' }}>CONTROLE FINANCEIRO PESSOAL</div>
        </div>

        {/* Tabs */}
        <div style={{ display: 'flex', background: T.tabBg, borderRadius: 12, padding: 4, marginBottom: 24, border: `1px solid ${T.tabBorder}`, transition: 'all .3s' }}>
        ...
        <form onSubmit={handleSubmit}>
          {mode === 'register' && (
            <div style={{ marginBottom: 14 }}>
              ...
              <input style={{ width: '100%', background: T.inputBg, border: `1px solid ${T.inputBorder}`, borderRadius: 10, color: T.text, padding: '11px 14px', fontSize: 13, fontFamily: 'inherit', transition: 'all .2s' }}
