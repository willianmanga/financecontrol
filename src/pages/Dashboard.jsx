...
function Toast({msg,type}) {
  ...
  return (
    <div style={{position:'fixed',bottom:24,right:16,zIndex:999,background:'rgba(7,11,20,0.97)',border:`1px solid ${c}44`,borderRadius:14,padding:'12px 18px',display:'flex',alignItems:'center',gap:10,boxShadow:`0 8px 32px ${c}22`,animation:'fadeUp .3s ease',maxWidth:'calc(100vw - 32px)'}}>
...
function ThemeToggle({mode,setMode,isDark,T}) {
  return (
    <div style={{display:'flex',gap:3,background:T.input,borderRadius:10,padding:3,border:`1px solid ${T.border}`}}>
      {[['dark','🌙'],['light','☀️'],['system','💻']].map(([k,ic])=>(
        <button key={k} onClick={()=>setMode(k)} title={k}
...
function EditModal({expense,onSave,onClose,T,isDark}) {
  ...
  const save = async()=>{
    if(!name.trim()||!value||isNaN(parseFloat(value)))return
    setSaving(true)
    await onSave(expense.id,{name:name.toUpperCase().trim(),value:parseFloat(value),category})
    setSaving(false)
  }

  const inp={background:T.input,border:`1px solid ${T.inputBorder}`,borderRadius:10,color:T.text,padding:'10px 13px',fontSize:13,fontFamily:'inherit',outline:'none',width:'100%',boxSizing:'border-box'}
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',padding:16,backdropFilter:'blur(4px)'}} onClick={onClose}>
      <div style={{background:isDark?'rgba(10,14,26,0.99)':'rgba(255,255,255,0.99)',border:`1px solid rgba(99,102,241,0.3)`,borderRadius:20,padding:'28px',width:'100%',maxWidth:420,boxShadow:'0 32px 80px rgba(0,0,0,0.5)',animation:'fadeUp .2s ease'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
          <div style={{fontSize:16,fontWeight:800,color:T.text}}>✏️ Editar Despesa</div>
        ...
        <div style={{display:'flex',gap:10}}>
          <button onClick={onClose} style={{flex:1,background:'transparent',border:`1px solid ${T.border}`,borderRadius:10,color:T.textMuted,padding:'11px',fontWeight:700,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Cancelar</button>
          <button onClick={save} disabled={saving} style={{flex:2,background:'linear-gradient(135deg,#6366f1,#8b5cf6)',border:'none',borderRadius:10,color:'#fff',padding:'11px',fontWeight:800,fontSize:13,cursor:saving?'not-allowed':'pointer',fontFamily:'inherit',boxShadow:'0 4px 14px rgba(99,102,241,0.35)'}}>
...
function MobileNav({tab,setTab,T,isDark}) {
  const TABS=[['overview','⊞','Início'],['expenses','≡','Despesas'],['charts','◉','Gráficos'],['history','◷','Histórico']]
  return (
    <nav style={{position:'fixed',bottom:0,left:0,right:0,background:isDark?'rgba(8,11,22,0.98)':'rgba(248,250,255,0.99)',borderTop:`1px solid ${T.border}`,display:'flex',zIndex:200,backdropFilter:'blur(20px)'}}>
      {TABS.map(([k,ic,lb])=>(
...
export default function Dashboard({user}) {
  ...
  return (
    <div style={G.app}>
        ...
        {tab==='privacy' && (
          <div style={{animation:'fadeUp .3s ease',maxWidth:680}}>
            ...
            <div style={{...G.sec(T.border),lineHeight:1.8}}>
              ...
              <div style={{fontSize:11,color:T.textMuted,fontFamily:"'JetBrains Mono',monospace",marginBottom:8,letterSpacing:'1px'}}>ÚLTIMA ATUALIZAÇÃO: MARÇO/2026</div>
      ...
      </div>Effect, useCallback } from 'react'
import { supabase } from '../lib/supabase'
import { PieChart, Pie, Cell, Tooltip, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, AreaChart, Area, Legend } from 'recharts'
